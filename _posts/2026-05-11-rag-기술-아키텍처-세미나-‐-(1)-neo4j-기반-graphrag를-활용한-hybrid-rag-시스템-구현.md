...
현재 GraphRAG 생태계는 Microsoft의 오픈소스 기술과 Neo4j의 전용 라이브러리를 중심으로 발전하고 있습니다. Microsoft는 GitHub에 공개된 GraphRAG 오픈소스 기술을 Azure 기반 과학 연구 플랫폼인 'Microsoft Discovery'에 통합하여 고도화하고 있습니다. 반면, Neo4j는 공식 neo4j-graphrag-python 라이브러리를 출시하며 자사 그래프 데이터베이스(DBMS) 환경에서 고성능 GraphRAG를 가장 손쉽게 구현할 수 있는 최적의 개발 환경을 제공합니다.
...
### 4.4 Knowledge-based GraphRAG의 두 가지 하위 접근

① 스키마-프리(Schema-free) Knowledge-based
  - Microsoft GraphRAG가 대표 사례
  - 사전 정의된 온톨로지 없이, LLM이 자유롭게 엔터티와 관계를 추출
  - Leiden 알고리즘으로 커뮤니티 탐지 후 커뮤니티 요약 생성
  - 전역 질의(global query)에 강점

② 온톨로지-기반(Ontology-driven) Knowledge-based  ← 본 세미나의 방식
  - 사전 설계된 도메인 온톨로지(스키마)로 추출 범위와 관계 유형을 제한
  - 일관성과 정밀도가 높음
  - 도메인 특화 탐색에 강점

> **Index-based GraphRAG**(관련글 [Knowledge-based GraphRAG vs. Index-based GraphRAG](https://k82022603.github.io/posts/knowledge-based-graphrag-vs.-index-based-graphrag/))는 별개의 패러다임으로, 원문 청크를 그대로 보존하고 그래프는 검색 안내 역할만 수행합니다. (대표 사례: RAPTOR, HippoRAG)
...
LLM이 "관련 문서에 따르면"이 아니라, "A 시스템은 B 라이브러리를 사용하고, 해당 버전은 C 취약점의 영향을 받으므로"처럼 **관계 기반 근거**를 명시하며 응답할 수 있습니다. 이는 환각(Hallucination)을 줄이고 응답의 신뢰도를 높입니다. Microsoft Research(2024)에 따르면 "전역 질의 포괄성 72~83% 향상"이 있습니다.
### 9.4 아키텍처팀 업무 특화 지원
