...
```mermaid
flowchart LR
    subgraph MS_PHIL["Microsoft GraphRAG 철학\n'문서 전체를 요약해 두자'"]
        direction TB
        MP1["LLM이 모든 청크를 읽고\n엔터티·관계를 자유롭게 추출"]
        MP2["Leiden 알고리즘으로\n자동 커뮤니티 구성"]
        MP3["커뮤니티마다\nLLM이 요약 보고서 생성"]
    ...
    subgraph NEO_PHIL["Neo4j GraphRAG 철학\n'구조화된 관계를 저장하자'"]
        direction TB
...
flowchart TB
    ...
    subgraph MS_PIPE["Microsoft GraphRAG 인덱싱\n(graphrag index)"]
        direction TB
        MS1["① 텍스트 청크 분할\n(기본 1,200 토큰)"]
    ...
    subgraph NEO_PIPE["Neo4j GraphRAG 인덱싱\n(SimpleKGPipeline)"]
        direction TB
