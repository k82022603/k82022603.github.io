...
mermaid: [True]
tags: [AI,  MoAI-ADK,  claude-code,  GEARS,  SDD,  spec-driven-development,  agentic-coding,  Claude.write]
---


> 이 문서는 [Goos Kim](https://goos.kim/ko)이 공개한 MoAI-ADK v3.0 예고 내용, GEARS 명세 문법, 그리고 교보문고 컴퓨터/IT 분야 일간 베스트셀러 1위를 기록한 ["클로드 코드로 시작하는 실전 에이전틱 코딩"](https://aladin.kr/p/rSLvw) 책을 중심으로 현재 에이전틱 코딩(Agentic Coding) 생태계의 흐름을 상세히 풀어낸 해설입니다.

---

## 목차

1. [전체 맥락: 무슨 일이 일어나고 있는가](#1-전체-맥락)
2. [Claude Code와 MoAI-ADK란 무엇인가](#2-claude-code와-moai-adk란-무엇인가)
3. [EARS: 요구사항 표기법의 기원](#3-ears-요구사항-표기법의-기원)
4. [GEARS: AI 시대를 위한 명세 문법의 진화](#4-gears-ai-시대를-위한-명세-문법의-진화)
5. [GEARS 문법 완전 해설](#5-gears-문법-완전-해설)
6. [MoAI-ADK v3.0 주요 변경 사항 예고](#6-moai-adk-v30-주요-변경-사항-예고)
7. [터미널 화면 분석: Claude Code 실제 작동 모습](#7-터미널-화면-분석)
8. [핸드오프 프롬프트: 세션 연속성 관리 시스템](#8-핸드오프-프롬프트-세션-연속성-관리)
9. [5가지 작업 옵션 카드: 에이전트가 제안하는 다음 행동](#9-5가지-작업-옵션-카드)
10. [SDD(명세 주도 개발)이란 무엇이며 왜 중요한가](#10-sdd-명세-주도-개발)
11. [교보문고 베스트셀러 1위 달성](#11-교보문고-베스트셀러-1위)
12. [정리 및 시사점](#12-정리-및-시사점)

---

## 1. 전체 맥락

2026년 5월, 에이전틱 코딩(Agentic Coding) 생태계에서 한국 개발자 커뮤니티를 중심으로 의미 있는 움직임이 동시에 일어났다.

Anthropic의 터미널 기반 AI 코딩 도구인 **Claude Code** 위에서 동작하는 오픈소스 개발 프레임워크 **MoAI-ADK**의 세 번째 주요 버전(v3.0)이 예고됐고, 이와 함께 AI 코딩에 최적화된 새로운 명세 문법인 **GEARS**가 MoAI-ADK에 탑재될 예정임이 공개됐다. 동시에 이 프레임워크의 개념적 기반을 담은 책 "클로드 코드로 시작하는 실전 에이전틱 코딩"이 **교보문고 컴퓨터/IT 분야 온라인 일간 베스트셀러 1위**를 달성했다는 소식도 함께 전해졌다.

이 일련의 사건들은 단순한 도구 업데이트를 넘어, AI와 인간이 협업하는 소프트웨어 개발 방식이 어떤 방향으로 성숙해 가는지를 보여주는 하나의 단면이다. 이 문서는 그 맥락을 처음 접하는 사람도 이해할 수 있도록 배경부터 기술 세부 사항까지 상세히 설명한다.

---

## 2. Claude Code와 MoAI-ADK란 무엇인가

### Claude Code

Claude Code는 Anthropic이 만든 **터미널 기반 AI 코딩 도구**다. 웹 브라우저가 아닌 개발자의 명령줄(터미널) 환경에서 동작하며, 개발자가 자연어로 요청하면 파일을 읽고, 코드를 작성하고, 테스트를 실행하고, Git 커밋까지 처리한다. 2025년 Anthropic이 공식 출시한 이후 전 세계 개발자 커뮤니티에서 빠르게 확산됐다.

Claude Code의 핵심 강점은 단순한 코드 자동완성을 넘어 **에이전트(Agent)로서의 자율 실행 능력**이다. 즉, 한 번의 지시로 여러 파일을 분석하고, 의존성을 파악하며, 수십 개의 파일을 수정하는 일련의 작업을 스스로 처리할 수 있다.

### MoAI-ADK

**MoAI-ADK(Agentic Development Kit)** 는 모두의AI(modu-ai)에서 개발한 오픈소스 프레임워크다. Claude Code라는 기반 도구 위에 올라타서, AI 에이전트가 더 체계적이고 품질 높은 코드를 생성하도록 환경 자체를 설계하는 방식, 즉 **하네스 엔지니어링(Harness Engineering)** 패러다임을 구현한다.

> "인간은 방향을 잡고, 에이전트는 실행합니다."
> — MoAI-ADK의 핵심 철학

하네스(Harness)란 원래 말을 제어하기 위한 마구(馬具)를 뜻한다. AI 에이전트라는 강력하지만 통제가 어려운 실행자를 올바른 방향으로 이끌기 위한 **구조적 환경 전체**를 하네스라고 부른다. 개발자의 역할이 "코드를 직접 쓰는 사람"에서 "에이전트가 잘 실행할 수 있는 명세(SPEC), 품질 게이트, 피드백 루프를 설계하는 사람"으로 전환된다는 것이 이 프레임워크의 핵심 메시지다.

현재 MoAI-ADK의 주요 구성 요소는 다음과 같다.

- **24개의 전문 AI 에이전트**: 백엔드 전문가, 프론트엔드 전문가, 테스팅 전문가 등 역할별로 분리된 에이전트들
- **52개의 스킬(Skills)**: 각 에이전트가 수행할 수 있는 도메인별 지식 및 절차
- **TDD/DDD 품질 게이트**: 테스트 주도 개발 및 도메인 주도 개발을 자동으로 적용
- **Go 기반 단일 바이너리**: Python으로 작성된 약 73,000줄의 초기 코드를 Go로 완전히 재작성해 의존성 없이 모든 플랫폼에서 즉시 실행 가능
- **SPEC 중심 워크플로**: `/moai plan → /moai run → /moai sync` 세 단계의 명령어 체계

---

## 3. EARS: 요구사항 표기법의 기원

GEARS를 이해하기 위해서는 먼저 GEARS의 모태가 된 **EARS(Easy Approach to Requirements Syntax)** 를 알아야 한다.

EARS는 2009년 Alistair Mavin 외 연구자들이 롤스로이스(Rolls-Royce)에서 실무 요구사항 작성의 어려움을 해결하기 위해 고안한 표기법이다. 이 논문은 2019년 IEEE 국제 요구사항 공학 컨퍼런스에서 **10년 최고 영향력 산업 논문상**을 수상할 만큼 소프트웨어 공학 분야에서 폭넓게 채택됐다.

EARS의 기본 철학은 자연어로 쓴 요구사항이 너무 모호하다는 문제를 해결하는 것이었다. "시스템은 빠르게 응답해야 한다"는 식의 애매한 표현 대신, 정해진 키워드 패턴을 사용해 누가 읽어도 동일하게 이해할 수 있는 요구사항을 작성하자는 취지였다.

### EARS의 5가지 패턴

EARS는 상황에 따라 다섯 가지 별개의 패턴을 정의한다.

| 패턴 | 문법 구조 |
|------|-----------|
| **Ubiquitous** (항상 적용) | The `<system>` shall `<response>` |
| **State-driven** (상태 조건) | While `<precondition>`, the `<system>` shall `<response>` |
| **Event-driven** (이벤트 조건) | When `<trigger>`, the `<system>` shall `<response>` |
| **Optional feature** (선택 기능) | Where `<feature>`, the `<system>` shall `<response>` |
| **Unwanted behavior** (비정상 동작) | If `<trigger>`, then the `<system>` shall `<response>` |

예를 들어, "사용자가 로그인 버튼을 누르면 시스템은 인증을 처리해야 한다"는 요구사항은 다음과 같이 작성된다.

```
When the user clicks the login button, the system shall authenticate the credentials.
```

이처럼 **When(트리거)**, **shall(필수 동작)** 이라는 키워드를 고정함으로써, 요구사항 문서 전체에 일관성이 생기고 테스트 케이스를 작성하기도 훨씬 쉬워진다.

### EARS의 한계

그러나 EARS는 애초에 항공기 엔진 같은 **고수준 시스템 요구사항**을 기술하기 위해 설계됐다. 현대의 에이전틱 코딩에서는 시스템 전체 수준의 요구사항뿐 아니라, 개별 컴포넌트, 마이크로서비스, AI 에이전트 함수 하나하나에 대한 명세가 필요하다. 또한 소프트웨어 테스트 영역에서 널리 쓰이는 **BDD(Behavior Driven Development)의 Given-When-Then 패턴**과 EARS는 별개의 언어 체계여서, 같은 기능을 요구사항과 테스트 케이스에 각각 다르게 표현해야 하는 인지적 부담이 있었다.

---

## 4. GEARS: AI 시대를 위한 명세 문법의 진화

이러한 배경에서 Σ*(Sublang)가 제안한 것이 **GEARS(Generalized Expression for AI-Ready Specs)** 다. GEARS는 "AI 코딩에 적합하도록 일반화된 EARS"라는 뜻이며, 2026년 1월 14일 Medium과 Dev.to에 공식 논문 형태로 발표됐다.

### GEARS 탄생의 배경

Amazon의 AI 코딩 도구인 **Kiro**가 EARS를 SPEC 주도 개발 워크플로에 도입하면서, EARS가 에이전틱 코딩 생태계에서 주목받기 시작했다. 그러나 실제 AI 코딩에서 EARS를 사용해보면 몇 가지 마찰이 생겼다.

첫째, "the system shall"이라는 고정된 주어 표현은 개별 컴포넌트나 에이전트를 주어로 써야 하는 상황에서 어색하다. 둘째, 다섯 가지 패턴이 별도로 존재하면 LLM이 어떤 패턴을 선택해야 할지 판단하는 데 추가적인 토큰 비용이 발생한다. 셋째, 비정상 동작(에러 처리)을 위한 `If...then` 패턴이 구문적으로 특별 취급되지만 실제로 에러 처리는 단순히 또 다른 트리거-응답 쌍에 불과하다. 넷째, EARS와 테스트 케이스의 Given-When-Then은 별도 언어 체계여서 동시에 유지 관리할 때 부담이 크다.
...
GEARS는 단 하나의 통합 패턴으로 모든 상황을 커버한다.
...
대괄호 `[ ]`로 묶인 절은 선택적으로 사용하며, 어떤 절을 포함하느냐에 따라 EARS의 다섯 패턴이 자연스럽게 표현된다. 별도로 패턴 이름을 고를 필요가 없다.

### 4가지 핵심 키워드

아래 표는 GEARS의 키워드와 각각이 BDD의 Given-When-Then에 대응하는 방식을 보여준다.

| 키워드 | 역할 | GWT 대응 | 예시 |
|--------|------|----------|------|
| **Where** | 정적 사전조건 (설정, 기능 플래그, 환경) | Given (설정) | `Where debug mode is enabled` |
| **While** | 상태 기반 사전조건 (실행 중 변할 수 있는 조건) | Given (상태) | `While the connection is active` |
| **When** | 트리거 (동작을 시작하는 이벤트, 최대 1개) | When | `When the user clicks submit` |
| **shall** | 필수 행동 (주어가 반드시 수행해야 하는 것) | Then | `The form shall validate inputs` |

### Where vs While: 정적 조건과 동적 상태의 구분

GEARS에서 가장 중요한 개념적 구분 중 하나가 **Where**와 **While**의 차이다.

- **Where**는 실행 도중 변하지 않는 조건이다. 배포 환경, 기능 플래그(Feature Flag), 설정값처럼 한 번 정해지면 실행 내내 고정되는 것들이다.
- **While**은 실행 도중 언제든 바뀔 수 있는 조건이다. 서킷 브레이커의 상태, 사용자의 로그인 여부, 캐시 용량처럼 런타임에서 동적으로 변하는 것들이다.

```
# Where 사용 예: 프로덕션 환경이라는 사실은 실행 중 변하지 않음
...
![image_01](https://github.com/k82022603/k82022603.github.io/blob/main/assets/img/MoAI-ADK-01.webp?raw=true)
...
![image_02](https://github.com/k82022603/k82022603.github.io/blob/main/assets/img/MoAI-ADK-02.webp?raw=true)
...
![image_03](https://github.com/k82022603/k82022603.github.io/blob/main/assets/img/MoAI-ADK-03.webp?raw=true)
...
![image_04](https://github.com/k82022603/k82022603.github.io/blob/main/assets/img/MoAI-ADK-04.webp?raw=true)
