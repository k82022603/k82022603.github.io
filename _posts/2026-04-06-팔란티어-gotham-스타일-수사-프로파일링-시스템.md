...
mermaid: [True]
tags: [AI,  owl-ontology,  palantir-ontology,  palantir-gotham,  Neo4j,  FastAPI,  Next.js,  microsoft-graph,  Microsoft-GraphRAG,  POLE,  Cypher,  GDS,  CRIS,  criminal-intelligence-system,  microsoft-office,  Claude.write]
---
...
>**RDF는 데이터를 연결하는 문법이고,**
>**OWL은 그 데이터들이 지켜야 할 의미와 논리적 규칙을 부여하는 도구입니다.**

```mermaid
graph TD

    subgraph ABox ["인스턴스 레이어 (ABox - 실제 데이터)"]
        I1["홍길동 :Investigator"]
        I2["김철수 :Suspect"]
        I3["사건-2025-001 :Case"]
    ...
    subgraph TBox ["온톨로지 레이어 (TBox - 개념 정의)"]
        direction TB
        C1["owl:Class: Person"]
        ...
        P1["owl:ObjectProperty: investigates"]
        P2["owl:ObjectProperty: involvedIn"]
    ...
    style C4 fill:#E6E6FA,stroke:#9370DB,color:#000
    style P1 fill:#F5F5F5,stroke:#333,stroke-dasharray: 5 5
    style P2 fill:#F5F5F5,stroke:#333,stroke-dasharray: 5 5
...
![image_03](https://lens.usercontent.google.com/banana?agsi=CmdnbG9iYWw6OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAwZWI6MTplODA3ZjkxY2NjYWQzNjk2OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAxNmQ0MDcwMzM2ODowMDA2NGVlZWUwZTI4OGFmEAIYAQ==)

- **RDF: 데이터의 기본 골격 (Triple)**
Alice Brown (주어) -- worksIn (서술어) -- Acme Corp. (목적어)와 같은 트리플이 모여 오른쪽의 ABox(실제 데이터)를 구성합니다. 이는 기계가 이해할 수 있는 가장 단순하고 강력한 데이터 표현 방식입니다.
- **OWL: 논리적 설계도 (Ontology)** 
"모든 수사관은 공무원이다"와 같은 논리는 이미지 왼쪽의 TBox(스키마) 영역에 해당합니다.

>**표현력**: Employee는 Person의 subClassOf라는 정의를 통해, "직원은 사람이다"라는 논리적 관계를 명시합니다.
>
>**추론(Reasoning)**: 만약 Alice Brown이 rdf:type으로 Employee라고 선언되어 있다면, 추론 엔진은 OWL 설계를 근거로 "앨리스는 사람(Person)이다"라는 새로운 사실을 자동으로 도출해냅니다.


### 4.4 수사 도메인 온톨로지 예시 (OWL/Turtle 표기)

![image_01](https://lens.usercontent.google.com/banana?agsi=CmdnbG9iYWw6OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAwZWI6MTowMzk3NTcyNjg5M2JlNjI5OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAxNmQ0MDcwMzM2ODowMDA2NGVlZTgyYTI3NjZmEAIYAQ==)
...
![image_02](https://lens.usercontent.google.com/banana?agsi=CmdnbG9iYWw6OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAwZWI6MTphNmI5ZGU2Njc2OTI5YmU4OjAwMDA1NWNmZWM3MDAyNmQ6MDAwMDAxNmQ0MDcwMzM2ODowMDA2NGVlZTk5MjczYzFmEAIYAQ==)
